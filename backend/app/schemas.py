...
class CabangBase(ApiSchema):
    ...
    patokan: str | None = Field(
        default=None,
        max_length=255,
    )

    nomor_whatsapp: str | None = Field(
        default=None,
        max_length=30,
    )
...
class KamarBase(ApiSchema):
    ...
    harga_bulanan: int = Field(
        gt=0,
    )

    periode_harga: str = Field(
        default="bulan",
        min_length=1,
        max_length=50,
    )

    jumlah_kamar: int = Field(
        default=1,
        ge=0,
    )

    kamar_tersedia: int = Field(
        default=0,
        ge=0,
    )

    ukuran: str | None = Field(
        default=None,
        max_length=100,
    )
    ...
    @classmethod
    def serialize_facilities(
        cls,
        value: Any,
    ) -> list[str]:
        return normalize_facilities(value)
...
class KamarUpdate(BaseModel):
    ...
    harga_bulanan: int | None = Field(
        default=None,
        gt=0,
    )

    periode_harga: str | None = Field(
        default=None,
        min_length=1,
        max_length=50,
    )

    jumlah_kamar: int | None = Field(
        default=None,
        ge=0,
    )

    kamar_tersedia: int | None = Field(
        default=None,
        ge=0,
    )
    ...
    def validate_availability(self):
        if (
            self.jumlah_kamar is not None
            and self.kamar_tersedia is not None
            and self.kamar_tersedia
            > self.jumlah_kamar
        ):
            raise ValueError(
                "Kamar tersedia tidak boleh "
...
class PengaturanUpdate(BaseModel):
    ...
    hero_cta_primary: str | None = Field(
        default=None,
        min_length=1,
        max_length=80,
    )

    hero_cta_secondary: str | None = Field(
        default=None,
        min_length=1,
        max_length=80,
    )

    cta_heading: str | None = Field(
        default=None,
        max_length=255,
    )
